...
const CODE = ['Barcode.gs', 'Code.gs'].map(f => fs.readFileSync(path.join(__dirname, '../apps-script', f), 'utf8')).join('\n');
...
const same = (dev, label) => {   // অফলাইন কপি == Google Sheets (সব ট্যাব)
  // আসল Sheets-এর getValues() আয়তাকার (সব সারি হেডারের সমান চওড়া) — নকল সার্ভারের এলোমেলো সারি সমান করে মেলানো
  const pad = rows => { const w = rows[0].length; return rows.map(r => r.concat(Array(Math.max(0, w - r.length)).fill(''))); };
  ['Products', 'Sales', 'Expenses', 'Savings', 'Settings'].forEach(t => {
    const ok_ = JSON.stringify(pad(dev.eng._store.tables[t])) === JSON.stringify(pad(server.store.tables[t]));
    ok(ok_, label + ': ' + t + ' স্থানীয় == সার্ভার');
  });
};
...
(async () => {
  ...
  /* ---- ৯ক) Google Sheet-এ বারকোড ছবি (সার্ভার-অ্যাকশন) ---- */
  {
    const before = server.call('getProducts').data.filter(p => !p.barcodeImageId && !p.barcodeImageUrl).length;
    const br = rpc({ key: KEY, action: 'barcodes' });
    ok(br.ok && br.data.made > 0, 'barcodes অ্যাকশন চলেছে: ' + (br.data && br.data.message));
    ok(!!server.store.tables.Barcodes && server.store.tables.Barcodes.length > 1, 'Sheet-এ "Barcodes" ট্যাব তৈরি হয়েছে');
    ok(rpc({ key: 'bad', action: 'barcodes' }).code === 'AUTH', 'ভুল Key দিয়ে বারকোড অ্যাকশন চলে না');
    A.st.online = true; await A.eng.syncNow();
    ok(A.eng.status().pending >= 0, 'বারকোড তৈরির পর ডিভাইস সিঙ্ক ঠিক আছে');
    const viaEngine = await A.eng.request('barcodes', {});
    eq(viaEngine.made, 0, 'Engine.request("barcodes") — সব ছবি আগেই আছে, নতুন ০');
    r = await A.call('getBarcodeImage', 'PRD-000001');
    ok(r.ok && r.data.mime === 'image/png' && r.data.b64.length > 200, 'অফলাইন ইঞ্জিনেই বারকোড PNG তৈরি হয় (ডাউনলোড/প্রিন্টের জন্য)');
  }
